...
echo "Generating Scheduler certs..."
...
openssl req -new -key zato.scheduler.key.pem -out zato.scheduler.req.csr -subj "/C=EU/ST=Zatoland/L=Zato City/O=Zato/CN=Zato Dev Scheduler"
openssl x509 -req -days 365 -in zato.scheduler.req.csr -CA ca_cert.pem -CAkey zato.ca.key.pem -CAcreateserial -out zato.scheduler.cert.pem
